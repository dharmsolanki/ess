      ...
      .then((data) => {
        if (data?.modal) {
          ...
          if (data?.modal?.TxtNetWrkHrs === "     ") {
            ...
            const [datePart, lastintime] =
              reversedArray[0]?.comments.split(" ");
...
function wikTime(Year, Month) {
  if (Year && Month) {
    const url = `https://ess.infibeam-inc.ooo/cosec/api/DailyAttendanceView/getDataForUserId?IsDatePicker=false&Para1=${Month}&Para2=${Year}&TemplateId=1&userId=${[
      localStorage["SecurityManager.username"],
    ]}`;
    fetch(url, {
      method: "GET",
...
function calculateRemainingTime(breakTime, initialTime, lastintime) {
  ...
  const result = {
    ...
    timeDifference: `${convertTo12HourFormat(
      sum.hours,
      sum.minutes
    )} `,
  };
...
function Todayhtml(remainingHours, remainingMinutes) {
  var today = new Date();
  var date =
    today.getDate() + "/" + (today.getMonth() + 1) + "/" + today.getFullYear();

  const formattedDate = formatDate(date);
  const message = `${remainingHours} hours and ${remainingMinutes} minutes.`;
